...
class AutonomousFlightNode(Node):
    def __init__(self) -> None:
        ...
        self.use_takeoff_land_topics = bool(
            self.get_parameter("use_takeoff_land_topics").value
        )
        self.takeoff_pub = self.create_publisher(
            Empty, self.get_parameter("takeoff_topic").value, 1
        )
        self.land_pub = self.create_publisher(
            Empty, self.get_parameter("land_topic").value, 1
        )

        self.alt_target = float(self.get_parameter("altitude_target_m").value)
        ...
        self.get_logger().info(
            f"Autonomous flight started with {len(self.search_waypoints)} waypoints."
        )

    def build_lawnmower_waypoints(self) -> List[Tuple[float, float, float, float]]:
        ...
        for y_row in y_rows:
            if left_to_right:
                waypoints.append((x_min, y_row, self.alt_target, self.start_yaw))
                waypoints.append((x_max, y_row, self.alt_target, self.start_yaw))
            else:
                waypoints.append((x_max, y_row, self.alt_target, self.start_yaw))
                waypoints.append((x_min, y_row, self.alt_target, self.start_yaw))
    ...
    def handle_takeoff(self) -> None:
        if self.use_takeoff_land_topics and not self.takeoff_sent:
            self.takeoff_pub.publish(Empty())
            self.takeoff_sent = True
            self.get_logger().info("Takeoff command published.")

        z = self.current_pose.position.z
        vz = self.clamp(self.kp_z * (self.alt_target - z), self.max_z_speed)
        self.publish_cmd(0.0, 0.0, vz, 0.0)

        if abs(self.alt_target - z) < self.alt_tol:
            self.publish_stop()
            self.state = MissionState.SEARCH
            self.get_logger().info("Reached takeoff altitude. Starting search mission.")

    def handle_search(self) -> None:
        if self.current_wp_idx >= len(self.search_waypoints):
            self.publish_stop()
            self.state = MissionState.LAND
            self.get_logger().info("Search pattern complete. Starting landing.")
            return

        tx, ty, tz, tyaw = self.search_waypoints[self.current_wp_idx]
        ...
        if (
            math.hypot(ex, ey) < self.xy_tol
            and abs(ez) < self.alt_tol
            and abs(yaw_error) < self.yaw_tol
        ):
            self.current_wp_idx += 1
            self.get_logger().info(
                f"Reached waypoint {self.current_wp_idx}/{len(self.search_waypoints)}."
            )
            self.publish_stop()
            return
    ...
    def handle_land(self) -> None:
        if self.use_takeoff_land_topics and not self.land_sent:
        ...
        z = self.current_pose.position.z
        if z > 0.12:
            vz = self.clamp(-self.kp_z * z, self.max_z_speed)
            self.publish_cmd(0.0, 0.0, vz, 0.0)
        else:
            self.publish_stop()
